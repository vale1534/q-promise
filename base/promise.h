...
static void runInObjectThread(QObject *object, F &&func) {
    ...
    auto pool = qobject_cast<QThreadPool *>(object);
    if (pool) {
...
public:
    ...
    static typename DeferObject<T>::Ptr create(ARGS &&...args) {
...
private:
    void _updateConnectionReceiver() {
        ...
        } else if (qobject_cast<QThreadPool *>(m_contextObject.data())) {
            // Thread explicitly given as context
            auto pool = static_cast<QThreadPool *>(m_contextObject.data());
            m_connectionReceiverWrapperPtr = QSharedPointer<ConnectionReceiverWrapper>::create(pool, false);
            return;
...
public:
    ...
    Promise<T>
        ...
        return _final<T, void>(std::move(func));
    }
...
private:
    ...
    template <typename PromiseType, typename RetValType, typename Functor>
    Promise<PromiseType> _final(Functor &&functor) const {
        static_assert(Private::function_traits<Functor>::arity <= 1, "_then(): Callback should take not more than one parameter");
